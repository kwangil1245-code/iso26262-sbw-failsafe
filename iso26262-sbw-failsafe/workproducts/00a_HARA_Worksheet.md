...
대상 시스템은 **조향 입력 → SteeringSensor → CAN → 조향 제어 SW → Actuator → Motor**로 이어지는 전체 조향 데이터 및 제어 흐름을 기준으로 하며, 다음 네 가지 위험 관점을 주요 분석 대상으로 한다.

* 조향 CAN 데이터 갱신 이상
* 비정상 조향 입력
* 조향 제어 관련 SW 실행 이상
* 운전자 의도와 일치하지 않는 비정상 조향 출력

본 문서에서는 구체적인 진단 메커니즘, AUTOSAR 모듈, Fault 판정 조건, FAIL-SAFE 복귀 조건 등의 구현 방법은 정의하지 않으며, 해당 내용은 후속 시스템 및 SW 요구사항·설계 단계에서 구체화한다.
...
| ------- | --------------------------------------------------------------- | ------------------------------------------------- | -- | -- | -- | --------------- | -------------------------------------------------------------------- |
| HC-01   | CAN 메시지 갱신 이상을 감지하지 못해 오래된 조향 입력이 계속 사용되고 운전자 의도와 다른 조향 출력이 발생함 | 주행 중 입력 ECU 또는 CAN 통신 이상으로 조향 데이터 갱신이 중단되는 상황     | S3 | E4 | C3 | D (Provisional) | **SG-01:** 조향 CAN 데이터가 정상적으로 갱신되지 않는 경우 이를 감지하고 시스템을 안전 상태로 전환해야 한다. |
| HC-02   | 비정상 조향 입력이 정상 입력으로 처리되어 운전자 의도와 다른 조향 출력이 발생함                   | 주행 중 센서 오류 또는 비정상적인 조향 입력이 발생하는 상황                | S3 | E4 | C3 | D (Provisional) | **SG-02:** 비정상 조향 입력을 감지하고 해당 입력으로 인해 위험한 조향 출력이 발생하지 않도록 해야 한다.     |
| HC-03   | 조향 제어 관련 SW 실행 이상을 감지하지 못해 오래된 데이터가 사용되거나 비정상 조향 출력이 지속됨        | ECU 내부에서 조향 제어 기능의 주기 또는 실행 흐름에 이상이 발생하는 상황       | S3 | E3 | C3 | D (Provisional) | **SG-03:** 조향 제어 관련 SW 실행 이상을 감지하고, 이상 발생 시 시스템을 안전 상태로 전환해야 한다.     |
| HC-04   | 조향 제어 또는 출력 경로의 이상으로 운전자 의도와 일치하지 않는 조향 출력이 발생함                 | 정상적인 조향 입력이 제공되는 상황에서 제어 연산 또는 구동 출력에 이상이 발생하는 상황 | S3 | E4 | C3 | D (Provisional) | **SG-04:** 시스템은 운전자의 조향 의도와 일치하지 않는 비정상적인 조향 출력이 발생하지 않도록 해야 한다.     |

## 4. Safety Goal 요약

| Safety Goal ID | Safety Goal                                               |
| -------------- | --------------------------------------------------------- |
| SG-01          | 조향 CAN 데이터가 정상적으로 갱신되지 않는 경우 이를 감지하고 시스템을 안전 상태로 전환해야 한다. |
| SG-02          | 비정상 조향 입력을 감지하고 해당 입력으로 인해 위험한 조향 출력이 발생하지 않도록 해야 한다.     |
| SG-03          | 조향 제어 관련 SW 실행 이상을 감지하고, 이상 발생 시 시스템을 안전 상태로 전환해야 한다.     |
| SG-04          | 시스템은 운전자의 조향 의도와 일치하지 않는 비정상적인 조향 출력이 발생하지 않도록 해야 한다.     |
