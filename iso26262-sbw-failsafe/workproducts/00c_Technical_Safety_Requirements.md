...
| :---: | --- | --- | :---: |
| **TSR_001** | 통신 진단 | 수신 ECU는 10ms 주기로 수신되는 조향 메시지의 Alive Counter를 감시해야 한다. 최초 정상 수신 이후 현재 Alive Counter가 (이전 정상 Alive Counter + 1) mod 256과 일치하지 않거나 데이터 수신에 실패한 경우 통신 Fault를 생성해야 한다. | FSR_001 |
| **TSR_002** | 입력 진단 | 수신 ECU는 수신된 조향각이 −512~511의 유효 범위에 포함되는지 검사해야 한다. 조향각이 유효 범위를 벗어나거나 데이터를 정상적으로 읽지 못한 경우 입력 Fault를 생성해야 한다. | FSR_002 |
| **TSR_003** | 실행 진단 | 수신 ECU는 조향 입력 감시, 안전 상태 판단 및 출력 제어 기능이 설정된 10ms 실행 주기 내에 수행되는지 감시해야 한다. 설정된 실행 주기 내에 필수 기능의 수행이 확인되지 않은 경우 실행 Fault를 생성해야 한다. | FSR_003 |
| **TSR_004** | Fault 관리 | 수신 ECU는 통신 Fault, 입력 Fault 및 실행 Fault를 종합하여 시스템의 정상 여부를 판단해야 한다. 하나 이상의 Fault가 활성화된 경우 Fail-Safe 전환을 요청해야 한다. | FSR_004 |
| **TSR_005** | 상태 관리 | 수신 ECU는 통신 Fault 또는 입력 Fault가 확인된 경우 Normal 상태에서 Fail-Safe 상태로 전환해야 한다. Fault 판정 결과는 늦어도 다음 10ms 제어 주기의 출력 갱신 전까지 반영되어야 하며, 정상 복귀 조건이 충족될 때까지 Fail-Safe 상태를 유지해야 한다. | FSR_004 |
| **TSR_006** | 안전 출력 | 수신 ECU는 Fail-Safe 상태에서 조향 모터에 전달되는 PWM Duty를 0%로 설정하고 좌·우 방향 제어 출력을 모두 비활성화해야 한다. 또한 Fail-Safe 상태에서 수신된 조향 입력값을 모터 제어에 사용해서는 안 된다. | FSR_005 |
| **TSR_007** | 상태 복귀 | 수신 ECU는 Fail-Safe 진입을 유발한 Fault 판정값을 정상 복귀 횟수에 포함해서는 안 된다. Fail-Safe 진입 이후 처음 수신한 정상값을 기준값으로 설정하고, 이후 Alive Counter가 (이전 정상값 + 1) mod 256 조건을 3회 연속 충족하며 조향각이 유효 범위에 포함되는 경우에만 Normal 상태로 복귀해야 한다. | FSR_004, FSR_005 |
