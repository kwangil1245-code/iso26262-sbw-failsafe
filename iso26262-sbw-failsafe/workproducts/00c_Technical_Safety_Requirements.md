...
| :---: | --- | --- | :---: |
| **TSR_001** | 통신 진단 | 수신 ECU는 10ms마다 수신되는 조향 정보의 Alive Counter를 감시해야 한다. 현재 Alive Counter가 (이전 Alive Counter+1) mod 256과 일치하지 않을 경우 통신 Fault를 생성해야 한다. | FSR_001 |
| **TSR_002** | 입력 진단 | 수신 ECU는 수신된 조향 입력을 정의된 유효 범위와 비교하여 입력의 유효성을 판단하고, 유효 범위를 벗어난 경우 입력 Fault를 생성해야 한다. | FSR_002 |
| **TSR_003** | 실행 진단 | 수신 ECU는 수신된 조향 입력이 −512~511 범위인지 검사하고, 범위를 벗어나거나 데이터 읽기에 실패한 경우 입력 Fault를 생성해야 한다. | FSR_003 |
| **TSR_004** | Fault 관리 | 수신 ECU는 통신 Fault, 입력 Fault 또는 실행 Fault가 확인되면 정의된 안전 반응시간 이내에 Normal 상태에서 Fail-Safe 상태로 전환해야 한다. | FSR_004 |
| **TSR_005** | 상태 관리 | 안전 관련 Fault가 확인된 경우 수신 ECU는 정상 조향 제어 상태에서 Fail-Safe 상태로 전환하고, Fault가 지속되는 동안 Fail-Safe 상태를 유지해야 한다. | FSR_004 |
| **TSR_006** | 안전 출력 | 수신 ECU는 Fail-Safe 진입 시 조향 Actuator에 전달되는 PWM 출력을 0%로 설정하고 좌·우 방향 출력을 비활성화해야 한다. | FSR_005 |
| **TSR_007** | 상태 복귀 | 수신 ECU는 Fail-Safe 진입을 유발한 값을 제외하고, 이후 처음 수신한 정상값을 기준으로 Alive Counter가 3회 연속 정상 증가한 경우에만 Normal 상태로 복귀해야 한다. | FSR_004, FSR_005 |
