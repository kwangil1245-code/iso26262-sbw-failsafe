...
|---|---|---|---|
| **NORMAL** | 조향 데이터 갱신 상태, 조향 입력 및 내부 실행 상태가 정상 | 유효한 조향 입력을 기반으로 조향 출력을 결정하고 출력 장치를 제어한다. | 데이터 갱신 이상, 비정상 입력 또는 내부 실행 이상 감지 |
| **FAIL-SAFE** | 데이터 갱신 이상, 비정상 입력 또는 내부 실행 이상 감지 | 위험한 조향 동작이 발생하지 않도록 조향 출력을 제한한다. | 안전 상태 진입 원인이 해소되고 정상 조건이 지속적으로 충족됨 |
...
FAIL-SAFE 상태에서 안전 상태 진입 원인이 해소되고, 조향 데이터 갱신 상태, 조향 입력 및 내부 실행 상태가 정의된 정상 조건을 지속적으로 만족하는 경우 NORMAL 상태로 복귀할 수 있다.
...
- Fault 판정 횟수, 유효 범위, 연속 정상 확인 횟수 등 구체적인 판단 기준은 SW 요구사항에서 정의한다.
- SWC, Runnable, RTE 인터페이스 및 내부 데이터 흐름은 SW 아키텍처 설계에서 정의한다.
- 함수, 상태 변수 및 상세 제어 로직은 SW 상세설계에서 정의한다.
- 시스템 요구사항과 시스템 기능, SW 요구사항 간 추적성을 유지한다.
