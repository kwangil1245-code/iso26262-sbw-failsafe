...
본 문서는 시스템 수준의 아키텍처를 다루며 SWC, Runnable, RTE API, 내부 변수, 진단 횟수 및 제어 알고리즘과 같은 소프트웨어 구현 상세는 정의하지 않는다. 해당 내용은 `03_SW_Requirements.md`, `04_SW_Architecture_Design.md`, `05_SW_Detailed_Design_Unit_Construction.md`에서 구체화한다.
...
| 시스템 처리 | 조향 정보 전달, 데이터 갱신 상태 감시, 입력 유효성 판단, 내부 실행 감시, 안전 상태 판단, 조향 출력 계산 및 제한           |
...
```mermaid
flowchart TD
    A["조향 입력 수집·전달"] --> B["조향 데이터 및 상태 감시"]
    B --> C["안전 상태 판단"]
    C --> D["조향 출력 결정"]
    D --> E["하드웨어 출력"]
```
...
| SYS-F-003   | 데이터 갱신 상태 감시 | 조향 정보가 정상적으로 수신 및 갱신되는지 판단한다.                | SYS-E-004                       |
| SYS-F-004   | 입력 유효성 판단    | 수신된 조향 입력이 정상적으로 사용할 수 있는 값인지 판단한다.          | SYS-E-004                       |
...
| SYS-F-007   | 안전 출력 제한     | FAIL-SAFE 상태에서 위험한 조향 출력이 발생하지 않도록 출력을 제한한다. | SYS-E-004, SYS-E-005            |
| SYS-F-008   | 정상 상태 복귀     | 정의된 정상 조건이 지속적으로 충족되는 경우 NORMAL 상태로 복귀한다.    | SYS-E-004                       |
| SYS-F-009   | 조향 출력 계산     | 유효한 조향 입력과 현재 시스템 상태를 기반으로 조향 제어값을 결정한다.     | SYS-E-004                       |
| SYS-F-010   | 조향 하드웨어 출력   | 결정된 조향 제어값을 출력 장치에 전달한다.                     | SYS-E-004, SYS-E-005            |
...
구체적인 정상 판정 조건 및 연속 정상 확인 횟수는 후속 SW 요구사항 및 상세 설계 단계에서 정의한다.
...
Alive Counter를 이용한 갱신 판단 방식, 조향 입력 유효 범위, WdgM을 이용한 실행 감시 방식, Fault 판정 횟수, 정상 복귀 횟수 등의 상세 요구사항은 `03_SW_Requirements.md`에서 정의한다.

SWC 구성, RTE Interface, Runnable 및 Task Mapping 등의 SW 구조는 `04_SW_Architecture_Design.md`에서 정의하며, 내부 변수, 상태 처리 로직 및 PWM 계산식 등의 구현 상세는 `05_SW_Detailed_Design_Unit_Construction.md`에서 관리한다.
