...
상위 시스템 요구사항에서 정의한 조향 정보 수신 상태 감시 기능은 SW 수준에서 Alive Counter를 이용한 데이터 갱신 상태 진단으로 구체화한다.

Fault가 확인된 경우 공통적으로 FAIL-SAFE 상태로 전환하고 위험한 조향 출력을 제한한다.

Fault가 해소된 이후에는 정의된 정상 상태 복귀 조건을 만족하는 경우에만 NORMAL 상태로 복귀한다.
...
SW 실행 상태 감시를 위한 구체적인 AUTOSAR WdgM 구성, Supervised Entity, Checkpoint 및 내부 연결 구조는 `04_SW_Architecture_Design.md`와 `05_SW_Detailed_Design_Unit_Construction.md`에서 정의한다.
...
정상 상태 복귀 확인 중 Fault가 다시 발생한 경우 정상 확인 누적 상태를 초기화하고 FAIL-SAFE 상태를 유지한다.

정상 조건 확인의 구체적인 시작 시점과 FAIL-SAFE 진입 시 사용된 입력값의 정상 확인 횟수 포함 여부는 본 요구사항에서 별도로 정의하지 않는다.
...
| `08_System_Verification.md` | 시스템 외부에서 관측 가능한 동작 | CAN Fault Injection, Invalid 입력, 실행 이상, FAIL-SAFE 및 정상 복귀 검증 |
...
| Port 및 Interface, RTE 연결 | `04_SW_Architecture_Design.md` |
...
| NORMAL / FAIL-SAFE 내부 상태 관리 로직 | `05_SW_Detailed_Design_Unit_Construction.md` |
...
NORMAL / FAIL-SAFE 상태는 안전 판단 및 조향 제어를 위한 내부 상태로 관리하며, 별도의 외부 상태 표시 기능은 본 프로젝트 범위에 포함하지 않는다.
